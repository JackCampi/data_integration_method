...
\paragraph{Hierarchic Probabilistic Model} is the first step to be defined, so we start with the bayes approach by treating parameters as random variables depending on some other random variables we call priors. i.e $\theta  \sim P(\theta \mid \lambda)$ and $\lambda \sim P(\lambda)$ 
