...
Kernel methods are algorithms which take the kernel function as an input. The kernel function can be seen as a "similarity function" that could describe how similar are two data points. The idea behind the kernel methods is to  map the data into higher-dimensional spaces where it is nicer to do machine learning. Or, in other words, the points are mapped into a space where they are more easily separable. The curious thing is that in this new space the points are now represented by their kernel function. 

This approach represents several advantages for our problem since it allows us to map elements from any general space, meaning our points can be real numbers, chains of characters, images, etc. This is already good for our heterogenious type of data. But still the problem of the integration persists. The kernel methods have an approach for this. The Multiple Kernel Learning (MKL) methods compute a kernel matrix for each feature and then combines them into a final kernel matrix. This could be done by minimizing an objective function, or ...

% Ponerlo mas matematico?
% Explicar bien los MKL
% Poner referencias

The advantage for MKL methods is that the final kernel matrix can be used for downstream analysis, supporting the analyses of influencing elements in the prediction.
